...
const rankingStatusToDot = (status) => {
  return (
    '<div class="d-inline-block me-1 rounded-circle spinner-grow-sm ' +
    ((status) => {
      switch (status) {
        case "unknown":
          return "bg-warning";
...
const getColumnsDefinition = (table) => {
  ...
  columnData.push({
    ...
    hozAlign: "left",
    cssClass: "no-wrap",
    tooltip: "share price trend",
    formatter: (cell) => {
      let data = cell.getRow().getData();
      let rank_change =
        data["pp_history"][data["pp_history"].length - 2].rank -
        data["pp_history"][data["pp_history"].length - 1].rank;
      let rank_txt = "&nbsp;&nbsp;&nbsp;"
      rank_txt +=
        rank_change < 0
          ? " &#9660;&nbsp;&nbsp;" + rank_change
          ...
          : "=";
      let change =
        data["pp_history"][data["pp_history"].length - 1].price -
        data["pp_history"][data["pp_history"].length - 2].price;
      let txt =
        change > 0
          ? rankingStatusToDot("up")
          : change < 0
          ? rankingStatusToDot("down")
          : rankingStatusToDot("unknown");
      return txt + change.toString() + rank_txt;
    }
  });
  columnData.push({
    minWidth: 160,
  ...
  window.columns["types"][table["type"]].forEach((th) => {
    ...
    console.log(
      ...
      propertyName(th["key"]),
      "calcWidth",
      calcTextWidth(propertyName(th["key"])),
      "total",
      calcTextWidth(propertyName(th["key"])) + 5
    );
    let columnUpdate = {
...
const generateAllTables = () => {
  ...
  window.tables.forEach((tab) => {
    tab["tables"].forEach((t) => {
      document.querySelector("#" + tab["tab"]).innerHTML +=
        ...
        '<a class="text-decoration-none text-white collapsed" title="Show/Hide Columns" id="toggleFilter-' +
        t["id"] +
        ...
        "</span></div>" +
        '<div class="card-body p-0"><div class="collapse" id="collapse-' +
        t["id"] +
        '">' +
        ...
        '" data-table="' +
        t["id"] +
        '" data-toggle-type="all"> ' +
        '<label class="form-check-label" for="toggleAll-' +
        t["id"] +
        '">Toggle All</label>' +
        "</div></div>" +
        '<div class="col-auto">' +
        '<button type="button" class="btn btn-outline-danger" onclick="resetColumns(\'' +
        t["id"] +
        "')\">" +
        ...
        '<button disabled class="btn btn-danger" id="discard-' +
        t["id"] +
        '" data-target="' +
        t["id"] +
        '" onclick="discardTableEdit(this)">' +
        '<i class="bi bi-trash"></i> Discard</button> ' +
        '<button disabled class="btn btn-success" id="save-' +
        t["id"] +
        '" data-target="' +
        t["id"] +
    ...
    tab["tables"].forEach((t) => {
      download +=
        ...
        t["title"] +
        '<span class="float-end d-flex justify-content-center">' +
        '<a class="text-decoration-none text-white me-3" title="Export as JSON" href="javascript:exportTable(\'json\', \'' +
        t["id"] +
        "');\">" +
        'JSON <i class="bi bi-cloud-download"></i></a>' +
        '<a class="text-decoration-none text-white me-3" title="Export as CSV" href="javascript:exportTable(\'csv\', \'' +
        t["id"] +
        "');\">" +
  ...
  window.tables.forEach((tab) => {
    tab["tables"].forEach((table) => {
      document
        .querySelectorAll("#collapse-" + table["id"] + " input")
        .forEach((el) => {
...
const countToggles = (table) => {
  return document.querySelectorAll("#collapse-" + table + " .toggle-row input")
    .length;
};
...
const countVisibleToggles = (table) => {
  let count = 0;
  document
    .querySelectorAll("#collapse-" + table + " .toggle-row input")
    .forEach((el) => {
      if (el.checked) {
        count += 1;
      }
...
const setToggleAllState = (table) => {
  let visible = countVisibleToggles(table),
    toggle = document.querySelector("#toggleAll-" + table);
  console.log(
    "Currently visible",
    visible,
    "of",
...
const toggleAll = (table) => {
  ...
  document
    .querySelectorAll("#collapse-" + table + " div.row input")
    .forEach((el) => {
      if (el.checked !== state) {
        el.checked = state;
...
const generateColumnsDetails = () => {
  ...
  Object.keys(window.columns["keys"]).forEach((key) => {
    ...
    accordion +=
      '<div class="col rounded hover-dark p-2">' +
      '<div class="row">' +
      '<div class="col-4">' +
      '<div class="badge hover-blue border border-primary py-1 px-2 rounded-pill">' +
      column["name"] +
      "</div>" +
      "</div>" +
      '<div class="col-8">' +
